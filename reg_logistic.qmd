...
table(lac$miserepro, lac$n_early_chronic_scc)
```
...
myreg_miserepro_scaled <- glmer(miserepro ~ n_early_chronic_scc + n_early_brief_scc + n_early_mam_cl + factor(C8parity) + early_milk + (1 | herd_id), 
...
```{r regression-lineaire-proba-un ia1 iaf superieur a 42, eval = FALSE}
...
```{r regression lineaire pour la probabilité d avoir un ia1 iaf superieur a 63, eval = FALSE}
...
```{r reg-lineaire pour la probabilité d avoir un ia1 iaf superieur a 84, eval = FALSE}
...
```{r reg-lineaire-proba ia1 iaf superieur a 84, eval = FALSE}
...
```{r regression lineaire pour la probabilité d avoir un ia1 iaf superieur a 84, eval = FALSE}
