...
## Obsrvations conservées pour l'analyse
...
Une nouvelle variable parité appelée `parKat` est créée. Les patités supérieures à 3 sont mises dans une catégorie 4. La paritté 2 est mise en catégorie de référence pour faciliter la lecture des résultats.
...
ggplot(prd, aes(x = factor(mast_cat1, level = level_order), y = estimate)) +
  ...
  xlab("") + 
  ylab("Probabilté de mise à la reproduction") +
