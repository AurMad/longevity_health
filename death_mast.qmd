...
```{r}
cox_model_essai <- coxph(Surv(time, death) ~ parKat + mast_cat1, data = lac, subset = n_ctrl_early > 0 & dp_mis == 0)
...
model_mast_cat1 <- coxme(Surv(time, death) ~ mast_cat1 + (1 | herd_id), data = lac, subset = n_ctrl_early > 0 & dp_mis == 0)
