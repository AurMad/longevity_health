...
t_cox1 <- tbl_regression(cox_model_essai1, exponentiate = FALSE)
print(t_cox1)
...
```{r}
t_cox <- tbl_regression(cox_model_essai1, exponentiate = FALSE)
print(t_cox)
```

```{r}
saveRDS(t_cox, "generated_datasets/t_cox.RData")
```
...
cox_mam <- ggplot(prd, aes(x = factor(mast_cat1, level = level_order), y = estimate)) +
  geom_point() +
...
```
```{r}
saveRDS(cox_mam, "generated_datasets/cox_mam.RData")
```

```{r}
parit_pred2 <- expand_grid(
  parKat = factor(1:4),
  time = 300,
  death = 0) |> 
  mutate( mast_cat1 = factor(ifelse(parKat == 1, "primi_healthy", "dpLow_healthy"))
)
...
cox_parit <- ggplot(prd, aes(x = factor(parKat), y = estimate)) +
  geom_point() +
...
```
```{r}
saveRDS(cox_parit, "generated_datasets/cox_parit.RData")
```
