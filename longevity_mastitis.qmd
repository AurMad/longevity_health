...
@Joanna : 
- Je mets un eval = false sur certains chunks. De cette manière, les chunks ne sont pas évalués quand tu exécutes le fichier ('render').
- Il est recommandé de nommer les chunks pour faciliter la lecture quand on extrait le code R.
- Eviter de modifier les jeux de données par les numéros de colonnes. C'est peu lisible et ça favorise les erreurs.
- Charger les packages en début de code. Encore une fois, c'est plus lisbile. Ca évite de les charger plusieurs fois.



Etablissons le lien entre mammites et mise à la reproduction (décision prise par l'éleveur). n_scc_early_high donne le nombre de contrôles pour lesquelles le taux de cellules est supérieur à 200 000 dans les 91 premiers jours de lactation. On va donc émettre l'hypothèse que si n \>= 2 alors la vache à fait un épisode de mammite chronique et ce qu'on cherche à savoir c'est si ce résultat à un impact sur la décision par l'éleveur d'inséminer sa vache.
...
Le code suivant doit faire la même chose que celui du chunk précédent.

-   Une variable `miserepro` est créée. Elle vaut 1 si le nombre d'IA est supérieur à 0 ce qui signe une décision de l'élveur de mettre la vache à la reproduction.
- une variable `mamchro`  est créée. Elle vaut 1 pour les lactations qui ont plus de 2 CCS supérieures à 200 000 cellules sur les 3 premiers contrôles du début de lactation.

```{r miserepro-mamchro-definition}
lac <- lac |> 
  mutate(miserepro = ifelse(ai_N == 0 | is.na(ai_N), 0, 1),
         mamchro = ifelse(n_scc_early_high >= 2, 1, 0))
```
...
- Je te conseille de faire cette opération directement dans le code ci-dessous, sans créer de jeu de données intermédiaire. Encore une fois, c'est plus lisbible.
...
@Joanna : je ne suis pas sûr que ce code fasse ce que tu veux qu'il fasse. Tu regardes la proportion de vaches mises à la repro quand mamchro = 1. Il me semble qu'il faudrait comparer mamchro = 0 et mamchro = 1. Voir mon code ci-dessous.
Je n'aime pas trop les camemberts en tant que représentations graphiques. Préférer des diagrammes en barres.

--> oui je vais essayer de modifier ça, effectivement il est plus pertinent de comparer mamchro = 1 et mamchro = 0 
...
--> il y a un problème avec ce tableau les valeurs de mise à la repro ne correspondent pas à celles du tableau lac

```{r}
#comparaison de la mise à la reproduction selon la présence ou non d'une mammite chronique en début de lactation
tab_repro_mam <- lac |> 
  select(miserepro, mamchro) |> 
  table()
  head(tab_repro_mam)
```
je me suis permise d'en créer un nouveau pour refaire un essai : 


```{r}
tab_repro_mam_1 <- lac |> 
  select(miserepro,mamchro) |> 
  mutate(miserepro = as.character(miserepro), mamchro = as.character(mamchro))
```
```{r}
str(tab_repro_mam_1)
```
...
xlab("mise à la repro selon mammite chronique ou non") +
ylab("Proportion") +
labs(fill = "miserepro")
p+theme_bw()
```
...
Ci-dessous, les proportions de vaches mises à la repro en fonction de la présence d'une mammite chronique. La relation est conforme à ce qui est attendu. Utilisation d'une fonction apply. Je pourrai t'expliquer comment ça fonctionne.

--> je ne comprend pas ce que ce chunk est censé faire. Si j'ai bien compris la fonction apply() applique la même fonction a tout les élements d'une ligne ou d'une colonne. Mais pourquoi faire la proportion de vaches mises à la repro pour chaque colonne ? 

```{r}
apply(tab_repro_mam, 2, function(x) round(x / sum(x), 3))
```
...
```{r simple-log-reg}
myreg <- glm(miserepro ~ mamchro, family= binomial(link = "logit"), data = lac)
summary(myreg)
```

On peut regarder l'odds-ratio de la mise à la repro en fonction du statut.
...
exp(coef(myreg)[1])
```
Version mixed de ce modèle avec effet aléatoire animal. Pas forcément pertinent. Idéalement, il faudrait mettre l'élevage en effet aléatoire. La colonne correspondante devra être ajoutée.

```{r mixed-log-reg}
myreg_mixed <- glmer(miserepro ~ mamchro + (1 | anim_id) , 
...
Ce résultat nous indique que le choix de la mise à la reproduction est influencé par la présence ou non de mammite lors des 91 premiers jours de lactation. Or pour ce premier modèle j'ai totalement ignoré les autres variables qui peuvent influencer ce résultat comme : - la parité - la production laitière - la présence d'une autre affection concomitante


rajout d'une nouvelle variable à ce modèle : prenons le rang de lactation 

```{r}
#rajout de la variable parité dans la régression logistique 
myreg2 <- glm(miserepro ~ mamchro + parity, family= binomial(link = "logit"), data = lac)
summary(myreg2)
```

Le coefficient semble logique, plus la parité augmente et moins la probabilité de mise à la repro est élevée. On voit également que la valeur est très significative 

Il faudrait égalemement ajouter la production laitière (simplement la production en kg sans prendre en compte le TB et le TP)
le problème c'est que la production laitière se trouve dans le data frame rec qui ne se présente pas de la même manière. 
Il faudrait peut-être calculer la moyenne de la PL sur l'ensemble des contrôles fait avant 91j de lactation et se servir de la moyenne dans le calcul ? 
...
lacpl <- inner_join(lac |> 
                     select(anim_id, parity, miserepro, mamchro, early_plmoy), 
                   rec_plmoy_parite |> 
                     select(anim_id, herd_id, parity, plmoy),
                   join_by(anim_id, parity)) |> 
  select(herd_id, anim_id, parity, early_plmoy, plmoy, mamchro, miserepro)
  
```

```{r}
myreg3 <- glm(miserepro ~ mamchro +factor(parity) + plmoy, family= binomial(link = "logit"), data = lacpl)
summary(myreg3)
```

```{r}
#nouvelle régression mais avec early_plmoy 
myreg4 <- glm(miserepro ~ mamchro +factor(parity) + early_plmoy, family= binomial(link = "logit"), data = lacpl)
summary(myreg4)
```


ajout de la variable aléatoire de troupeau

```{r}
myreg_mixed2 <- glmer(miserepro ~ mamchro + factor(parity) + early_plmoy + (1 | herd_id) , 
