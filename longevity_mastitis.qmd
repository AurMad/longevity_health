...
```{r}
# première tentative de régression logistique
...
```{r}
#Création d'un nouveau jeu de données qui donne la production laitière moyenne par lactation
...
Regroupement des parités selon des classes plus pertinentes : On voit que sur la régression linéaire, le résultat est significatif jusqu'a une parité de 10 on peut donc regrouper les classes de parité 9 et plus. 

```{r}
#Création d'une nouvelle variable parité en 9 classes
lacpl <- lacpl |> 
...
```
```{r}
#Régression logistique finale 2 pour la décision de mise à la repro
myreg_mixed3 <- glmer(miserepro ~ mamchro + factor(C9parite) + early_plmoy + (1 | herd_id), 
                    family= binomial(link = "logit"), data = lacpl)
