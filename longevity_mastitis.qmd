...
- une variable `early_mamchro`  est créée. Elle vaut 1 pour les lactations qui ont plus de 2 CCS supérieures à 200 000 cellules sur les 3 premiers contrôles du début de lactation.
...
Dans le tableau "mvt" on considère M = mort et E = équarissage qui est équivalent à mort

on considère cette fois 3 types de mammites : 
-mammite chronique de début de lactation (2 ctrl scc > 200 000)
-mammite chronique de fin de lactation (idem)
-mammite aiguë (1 scc < 200 000 suivi d'un scc > 200 000 et a nouveau suivi d'un scc < 200 000)
...
```{r}
scc_nxt = lead(scc),
         scc_aig = ifelse(scc >= 200 & scc_prev < 200 & scc_nxt < 200, 1, 0))
```
