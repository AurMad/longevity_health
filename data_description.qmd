...
Le fichier s'appelle `condition_naiss.txt`.
Il contient des informations sur des bovins, notamment des années et conditions de naissance.
...
Le fichier est assez volumineux (241 Mo).
Données classiques.
...
Il contient de données pour `r nrow(rec)` contrôles élemntaires sur `r  length(unique(rec$ANIM))` vaches.
On remonte assez loin en termes de dates de contrôles.
...
Ce fichier contient moins de lignes et plus de colonnes que le précédent.
...
Il y a 3 codes mamites différents.
Pas de description.
...
glimpse(mvt)
```
commentaire de Joanna : c'est à partir d'ici que j'ai commencé les modifications 
...
```{r Chargement du fichier rec-final}
...
on pourra essayer de faire 2 sous-groupes : - le fait d'avoir 2 controles successifs ou plus \> 200 000 juste avant l'IA - le fait d'avoir le dernier controle de la lactation précédente et le premier controle de cette lactation \> 200 000

```{r creation variable qui donne la valeur du contole suivant}
rec_final <- rec_final |> 
...
```{r creation variable qui compare ces valeurs}
rec_final <- rec_final |> 
...
```{r date de collecte mammite date de controle}
...
```{r}
mamt <- mamt |> 
  mutate(mamt_months = format(ctrl_date, "%Y-%m"))

mamt$mamt_months = as.Date(mamt$mamt_months, "%Y-%m")
```


```{r}
mamt2 <- mamt |> 
  mutate(mamt_months = as.Date(mamt_months,format = "%Y-%m"))
```


```{r}
mamt1 <- mamt |> 
 select(anim_id, ctrl_date, mamt_months)
```
...
J'ai renommé la date des collectes de mammites ctrl_date" car ces dates semblent correspondre aux dates de passage du controle laitier. j'arrive à ajouter comme ca la date de mammite quand il y en a


ERREUR DETECTEE : Certaines vaches comme par exemple "FR2534748533" possèdent des données en doublons pour les informations du contrôle laitier"...je ne sais pas pourquoi car il n'y a aucun doublon dans "rec" ni dans "rec1" donc le problème est lié à la modification de "rec_final". je pense que le problème vient du left_join...pour les vaches qui ont plusieurs mammites cliniques d'enregistrées sur un meme mois voire plusieurs jours qui se suivent, il duplique les contrôles/ou remplace certains contrôles....il faudrait que chaque enregristrement de mammites cliniques qui se suivent sur le mois soient considérés comme un seul et même épisode de mammite. 
SOLUTION : j'ai créer une variable ctrl_month en fonction des dates de controle et j'ai supprimé les doublons. Lorsqu'il y a eu plusieurs mammites cliniques enregristrées le même mois je considère donc que c'est le même épisode
...
les dates semblent correspondre.
On remarque aussi que les mammites cliniques semblent parfois correspondre à un pic de scc, parfois elle arrive après plusieurs controles sup à 200.
On va commencer à essayer de faire des catégories de mammites...Pour l'instant c'est fastidieux...

```{r}
rec_final <- rec_final |> 
  mutate(mam_cl = ifelse(mamt_months != is.na(mamt_months), 1, 0)) |> 
...
```
ERREUR DETECTEE : Certaines vaches possèdent des données en doublons pour les informations du contrôle laitier"...je ne sais pas pourquoi car il n'y a aucun doublon dans "rec" ni dans "rec1" donc le problème est lié à la modification de "rec_final". j'aimerais rectifier le problème à la source mais je ne le trouve pas...

## enregistrement de rec_final

```{r enregistrement de rec-final}
write.csv2(rec_final, "generated_datasets/rec_final.csv")
```

## Sélection d'un échantillon

Cette partie ne sert qu'à sélectionner un échantillon parmis toutes les données du contrôle laitier
...
Commentaire joanna pour Aurélien : J'ai trouvé pourquoi on a une perte de données dans lac, c'est parce que le filtre appliqué à rec qui enlève les années où les élevages ont moins de 11 contrôles, a coupé la lactation de certaines vaches qui chevauchent ces années.
Par exemple pour la vache "FR1456171629", les données débutent à partir de janvier 2012 dans rec_final, or cette vache à débuté sa lactation en 2011 donc on perd pas mal d'information notamment sur tout son début de lactation.
Or je ne sais pas comment retirer ses animaux du tableau car ils faussent les résultats.
le code comprend ses animaux comme n'yant pas de cellules élevés en début de lactation alors qu'on a juste pas l'information...(voir code suivant pour enlever les vaches dont on a pas pu calculer la PL en début de lactation)

```{r lac-list of lactations}
rec_final$out_cause[rec_final$anim_id == "FR2538643995"] <- NA
rec_final$out_date[rec_final$anim_id == "FR2538643995"] <- NA

## Lac_test permet de contrôler qu'on ne perde pas de données lors des modifications du tableau lac
lac_test <- rec_final |> 
...
lac <- rec_final |> 
  ...
  summarize(parity = unique(parity),
            ...
            n_mam_cl = length(mam_cl[mam_cl == 1]))
...
```{r tri des animaux dont on a pas d'information pour le debut de lactation}
lac <- lac[!is.nan(lac$early_milk),]
```

```{r estimation de la date du prochain velage}
lac <- lac |> 
  group_by(anim_id) |> 
...
```{r selection des dates ia comprises entre le velage precedent et le velage suivant, pour un meme animal et pour une meme lactation il y a plusieurs dates ia car elle a ete inseminee plusieurs fois}
...
```{r}
lac <- lac |> 
  filter(calv_date > as.Date("2009-01-01"))
```
...
```{r création sous-groupe de lac, eval=FALSE}
...
# Impact des mammites sur la fertilité

Cette partie est encore à retravailler. Je ne suis pas encore sûre de comment bien évaluer la fertilité grâce à nos données. 
plusieurs options peuvent être utilisées :
- nombre d'IA avant la fécondation ou non ?
- nombre de cycles avant fécondation ou non ?
- IVV ? 
- IVIAF ? 

## tri des animaux qui ont été mis à la reproduction

```{r tableau qui ne prend que les animaux mis à la repro et reformes en boucherie}
fertility_lac <- lac |> 
  filter(miserepro == 1 & out_cause == "B")
```

```{r}
fertility_lac <- fertility_lac |> 
  mutate(preg_end = ifelse(calv_nxt == as.Date(calv_nxt), 1, 0))

fertility_lac$preg_end[is.na(fertility_lac$preg_end)] <- 0 
```

```{r calcul du temps entre le velage et ia fecondante}
fertility_lac <- fertility_lac |> 
  mutate(lac_id = as.character(lac_id))

fertility_lac <- fertility_lac |> 
  group_by(lac_id) |> 
  mutate(dim_iaf = difftime(ai_last, calv_date, units = c("days")))
```

COmmentaire : Certains animaux ont un intervalle supérieur à 900 jours, très étrange il faut demander à IDELE ou regarder si dans le code quelque chose entraine ça.

```{r tri des animaux qui ont un intervalle inferieur a 300 jours}
fertility_lac <- fertility_lac |> 
  mutate(dim_iaf = as.numeric(dim_iaf))

fertility_lac <- fertility_lac |> 
  filter(dim_iaf <= 300)
```

```{r calcul de intervalle entre ia1 et iaf }
fertility_lac <- fertility_lac |> 
  mutate(iap_iaf = difftime(ai_last, ai_first, units = c("days")))

fertility_lac <- fertility_lac |> 
  mutate(iap_iaf = as.numeric(iap_iaf))
```

```{r creation de plusieurs categories pour les intervalles}
fertility_lac <- fertility_lac |> 
  mutate(inter_42 = ifelse(iap_iaf >= 42, 1, 0),
         inter_63 = ifelse(iap_iaf >= 63, 1,0),
         inter_84 = ifelse(iap_iaf >= 84, 1, 0 ))
```

```{r}
fertility_lac <- fertility_lac |> 
  mutate(n_ia2 = ifelse(ai_N == 2, 1, 0),
         n_ia3 = ifelse(ai_N == 3, 1,0),
         n_ia4_plus = ifelse(ai_N >= 4, 1, 0 ))
```

```{r, eval=FALSE}
write.csv2(fertility_lac, "generated_datasets/fertility_lac.csv")
```

# impact de la baisse de fertilité sur la réforme

Cette partie ne sert qu'à la création d'un tableau de données qui regroupent les animaux réformés en boucherie par l'éleveur .

```{r selection des vaches mises a la repro}
lac_repro <- lac|> 
  filter(miserepro == 1)
```
...
lac_repro <- lac_repro |> 
  group_by(anim_id) |> 
  mutate(cull = ifelse(out_cause == "B" & parity == max(parity), 1, 0))
```

```{r}
write.csv2(lac_repro, "generated_datasets/lac_repro.csv")
```
...
```{r}
ggplot(rec_final, aes(x = dim, y = milk)) +
  geom_point()
```
...
# effet de la baisse de la lactation sur la réforme

```{r retrait des vaches qui sont mortes au cours de la lactation}
lac_cull <- lac |> 
  filter(out_cause != "M")
```

```{r varaible qui affiche 1 si reforme a la fin de la lactation et 0 sinon}
lac_cull <- lac_cull |> 
  group_by(anim_id) |> 
  mutate(cull = ifelse(out_cause == "B" & parity == max(parity), 1, 0))
```
...
```{r enregistrement}
write.csv2(lac_cull, "generated_datasets/lac_cull.csv")
```
