...
```{r}
rec_final1 <- left_join(rec_final1,
                       mamt |> 
                         select(anim_id, ctrl_date, mamcl_date),
                       join_by(anim_id, ctrl_date)) |> 
...
rec_final1$mam_cl[is.na(rec_final$mam_cl)] <- 0
rec_final1$chronic_scc[is.na(rec_final$chronic_scc)] <- 0
rec_final1$brief_scc[is.na(rec_final$brief_scc)] <- 0
```
ERREUR DETECTEE : Certaines vaches comme par exemple "FR2534748533" possèdent des données en doublons pour les informations du contrôle laitier"...je ne sais pas pourquoi car il n'y a aucun doublon dans "rec" ni dans "rec1" donc le problème est lié à la modification de "rec_final". j'aimerais rectifier le problème à la source mais je ne le trouve pas...
