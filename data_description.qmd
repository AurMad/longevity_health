...
```{r rangement}
rec_final <- rec_final |> 
  select(herd_id, anim_id, parity, calv_date, ctrl_date, dim, milk, fat, prot, scc, scc_succ, scc_prev, chronic_scc, brief_scc )
```
...
test <- test |> 
  ...
  mutate(chronic_mam = ifelse(chronic_scc == 1 & mam_cl ==1, 1,0)) |> 
  mutate(brief_mam = ifelse(brief_scc ==1 & mam_cl ==1, 1,0))
```
...
lac <- rec_final |> 
  ...
  summarize(parity = unique(parity),
            ...
            n_chronic_scc= length(chronic_scc[chronic_scc == 1]),
            n_brief_scc = length(brief_scc[brief_scc == 1]),
