...
parit_pred <- expand_grid(
  ...
  mutate(
    ...
    milk_early_max_scaled_p3 = 0,
    milk_early_max_scaled_p4 = 0,
    mast_cat1 = factor(ifelse(parKat == 1, "primi_healthy", "multi_healthy"))
  )
```
...
```{r}
race_pred <- tibble(
  race = factor(race_levels),
  mast_cat1 = factor(c(rep("multi_healthy", length(race)))),
  parKat = factor(c(rep(3, length(race)))),
  milk_early_max_scaled_p1 = 0,
  milk_early_max_scaled_p2 = 0,
  milk_early_max_scaled_p3 = 0,
  milk_early_max_scaled_p4 = 0
...
write_csv2(mast_pred1, "generated_datasets/mast_pred1.csv")
...
```{r}
write.csv2(lac, "generated_datasets/lac_final.csv")
```
