...
Les données contrôle laitier et inséminations artificielles sont agrégées à l'échelle élevage-année. L'objectif est de sélectionner les élevages dans lesquels une proportion suffisante des vaches sont inséminées. Ici on compte le nombre de contrôle par élevage, le nombre de vaches contrôlées par élevage et la proporions de primipares contrôlées.
...
On va commencer par ajouter une variable d'année au tableau des lactations. Par année et par élevage on va calculer : - le nombre de vêlages - le nombre de vêlages de primipares - la proportion de vêlages de primipares sur le nombre de vêlage total - le nombre de premières inséminations - la proportion de premières inséminations sur le nombre de vêlages total
...
Le graphique ci-dessous donne en abscisse la proportions de vêlages de primipares et en ordonnées la proportions de premières inséminations par rapport au nombre de vêlages. Cela permet de faire une sélection pour avoir un groupe plus homogène.
...
On va également différencier les différentes périodes de survenue des mammites en se concentrant tout d'abord sur la période de début de lactation : - Les primipares débutent leur toute première lactation, elle n'ont pas encore connu de période sèche et n'ont pas encore eu l'occasion de se contaminer - Les multipares ont déjà pu se contaminer lors de la lactation précédente, on va donc prendre en compte son statut lors de la période sèche.
...
```{r ajout de terme quadratiques, eval = FALSE}
lac <- lac |> 
...
La figure ci-dessous représente les probabilités prédites qu'une vache soit mise à la reproduction en fonction de son statut vis-à-vis des mammites. Les multipares représentent la parité de référence qui est la parité 2.(J'aimerais le faire avec la parité 3 comme référence mais je ne sais pour quelle raison mon modèle ne converge plus...)

Bien évidemment on peut montrer l'impact de la parité seule sur la probabilité de mise à la reproduction

```{r}
prd <- predictions(model = repro_parity,
                   newdata = parit_pred, 
                   re.form = NA, type = "response")
...
Probabilité combinée pour la parité et la production laitière
...
```{r}
repro_revel_mast4 <- glmer(repro_revel ~  parKat + 
                              milk_early_max_scaled_p1 + 
                              milk_early_max_scaled_p2 + 
                              milk_early_max_scaled_p3 + 
                              milk_early_max_scaled_p4 + 
...
```{r}
vif(repro_revel_mast4)
```

## Probabilité de revêlage suite à la mise à la reproduction selon la parité

```{r, eval = FALSE}
prd <- marginaleffects::predictions(model = repro_revel_mast4,
                             newdata = parit_pred,
                           re.form = NA, type = "response")
...
```{r}
success_ia_full3 <- glmer(success_ai1 ~ parKat + 
                      (1 | herd_id),
                    data = lac,
                    subset = n_ctrl_early > 0 & repro == 1 & dp_mis == 0,
                    family = binomial(link = "logit"))

summary(success_ia_full3)
```

### Prédictions de la réussite à la première insémination selon le type de mammite

```{r}
prd <- marginaleffects::predictions(model = success_ia_full2, 
                             newdata = parit_pred, 
                           re.form = NA, type = "response")

ggplot(prd, aes(x = factor(mast_cat2, level = level_order), y = estimate)) +
  geom_point() +
  geom_segment(aes(x = mast_cat2, y = conf.low, xend = mast_cat2, yend = conf.high)) +
  xlab("") + 
  ylab("Probabilité d'être pleine suite à la première insémination") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
```
...
```{r}
ai_number_full <- glmer(success_ai1 ~ parKat + 
                              milk_early_max_scaled_p1 + 
                              milk_early_max_scaled_p2 + 
                              milk_early_max_scaled_p3 + 
                              milk_early_max_scaled_p4 + 
...
```{r}
ai_number_full1 <- glmer(success_ai1 ~ parKat + 
                              milk_early_max_scaled_p1 + 
                              milk_early_max_scaled_p2 + 
                              milk_early_max_scaled_p3 + 
                              milk_early_max_scaled_p4 + 
...
```{r}
ai_number_full2 <- glmer(success_ai1 ~ parKat + 
                      (1 | herd_id),
                    data = lac,
                    subset = !is.na(n_ai) & n_ai > 0 & dp_mis == 0,
                    family = poisson)

summary(ai_number_full2)
```
...
## Modèle sur l'impact de la reproduction sur la probabilité de réforme.

### Définition des variables

On va tout d'abord créer une variable qui code pour la réforme.

```{r creation cull}
lac  <- lac |> 
  mutate(cull = ifelse(out_cause_last == "B" & !is.na(out_date_last) & is.na(calv_date_nxt), 1, 0))
```
...
```{r mise à la repro - reforme}
p<-ggplot(lac |> 
            filter(n_ctrl_early > 0 & dp_mis == 0 & out_cause_last != "M")) +
aes(x = cull, fill = repro) +
geom_bar(position="fill") +
xlab("réforme") +
ylab("Effectifs") +
labs(fill = "décision de mise à la repro")
p+theme_bw()
## x = Nbre de groupe sur l'axe x
## fill = Nbre de barres par groupe
## position="fill" = rend le graphique proportionnel et empile les barres
...
```{r réussite à la première ia}
lac <- lac |> 
  mutate(success_ai1 = as.character(success_ai1))

p<-ggplot(lac |> 
            filter(n_ctrl_early > 0 & dp_mis == 0 & out_cause_last != "M" & !is.na(n_ai))) +
aes(x = cull, fill = success_ai1) +
geom_bar(position="fill") +
xlab("réforme") +
ylab("Effectifs") +
labs(fill = "succès à la première insémination")
p+theme_bw()
## x = Nbre de groupe sur l'axe x
## fill = Nbre de barres par groupe
## position="fill" = rend le graphique proportionnel et empile les barres
...
lac |> 
  filter(n_ctrl_early > 0 & dp_mis == 0 & out_cause_last != "M" & !is.na(n_ai)),
  x = "cull", y = "n_ai",
ggfunc = ggboxplot, add = "jitter", color = "cull", palette = "npg", xlab="réforme",
ylab="nombre d'ia")

p<-ggplot(lac |> 
  filter(n_ctrl_early > 0 & dp_mis == 0 & out_cause_last != "M" & !is.na(n_ai)))+aes(x= cull, y= n_ai) +
geom_boxplot(outlier.colour="black", outlier.shape=20,outlier.size=4,
fill='#99CCFF', color="black")+
stat_summary(fun.y=mean, geom="point", shape=16, size=4, col="red")+
xlab("réforme")+
ylab("nombre d'inséminations")
p+theme_bw()
```
...
```{r}
repro_cull_null <- glmer(cull ~ 1 + (1 | herd_id),
                    data = lac,
                    subset = n_ctrl_early > 0 & dp_mis == 0 & out_cause_last != "M",
                    family = binomial(link = "logit"))

summary(repro_cull_null)
```

on va obtenir ainsi la probabilité de réforme
...
# Extraire les coefficients fixes et calculer la probabilité
prob_cull <- as.double(round(invlogit(fixef(repro_cull_null)), 3))

# Afficher la probabilité
prob_cull
```

### modèle avec mise à la repro

```{r}
repro_cull_repro <- glmer(cull ~ repro + (1 | herd_id),
                    data = lac,
                    subset = n_ctrl_early > 0 & dp_mis == 0 & out_cause_last != "M",
                    family = binomial(link = "logit"))

summary(repro_cull_repro)
```

Ce modèle montre que la décision de mise à la repro diminue la probabilité de réforme ce qui est logique

### Modèle de réussite à la première insémination

```{r}
lac <- lac |> 
  mutate(success_ai1 = as.factor(success_ai1))
```

```{r}
repro_cull_ai1 <- glmer(cull ~ success_ai1 + repro + (1 | herd_id),
                    data = lac,
                    subset = n_ctrl_early > 0 & dp_mis == 0 & out_cause_last != "M",
                    family = binomial(link = "logit"))

summary(repro_cull_ai1)
```

```{r}
lac <- lac |> 
  mutate(n_ai = as.factor(n_ai))
```

```{r}
repro_cull_full <- glmer(cull ~ parKat + (1 | herd_id),
                    data = lac,
                    subset = n_ctrl_early > 0 & dp_mis == 0 & out_cause_last != "M",
                    family = binomial(link = "logit"))

summary(repro_cull_full)
```

```{r}
repro_cull_full1 <- glmer(cull ~ parKat + repro + (1 | herd_id),
                    data = lac,
                    subset = n_ctrl_early > 0 & dp_mis == 0 & out_cause_last != "M",
                    family = binomial(link = "logit"))

summary(repro_cull_full1)
```

```{r}
repro_cull_full2 <- glmer(cull ~ parKat + repro + milk_early_max_scaled_p1 + 
                              milk_early_max_scaled_p2 + 
                              milk_early_max_scaled_p3 + 
                              milk_early_max_scaled_p4 +
                              (1 | herd_id),
...
summary(repro_cull_full2)
```

```{r,  eval = FALSE}
repro_cull_full4 <- glmer(cull ~ factor(parKat) + factor(repro) + factor(success_ai1) + factor(n_ai)+ milk_early_max_scaled_p1 + milk_early_max_scaled_p2 + milk_early_max_scaled_p3 + milk_early_max_scaled_p4 +(1 | herd_id), data = lac,
          subset = n_ctrl_early > 0 & dp_mis == 0 & out_cause_last != "M",
          family = binomial(link = "logit"))

                    
summary(repro_cull_full4)
```
