...
## position="dodge" = position des barres à côté
```
```{r}
p<-ggplot(lac) +
aes(x = repro, fill = mast_cat1) +
...
repro_full4 <- glmer(repro ~ parKat + 
                              ...
                              milk_early_max_scaled_p4_sq + 
                              mast_cat1 +
                       parKat*milk_early_max_scaled_p1_sq +
                       parKat*milk_early_max_scaled_p2_sq +
                       parKat*milk_early_max_scaled_p3_sq +
                       parKat*milk_early_max_scaled_p4_sq +
                      (1 | herd_id),
                    data = lac,
                    subset = n_ctrl_early > 0 & dp_mis == 0,
                    family = binomial(link = "logit"))
